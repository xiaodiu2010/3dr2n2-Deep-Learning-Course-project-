...
class Affine(Augmenter):
    def __init__(self, scale=1.0, translate_percent=None, translate_px=None, rotate=0.0, shear=0.0, order=1, cval=0.0, mode="constant", name=None, deterministic=False, random_state=None):
        ...
        if order == ia.ALL:
            ...
            raise Exception("Expected order to be img_aug.ALL, int or StochasticParameter, got %s." % (type(order),))
        ...
        if cval == ia.ALL:
            ...
            raise Exception("Expected cval to be img_aug.ALL, int, float or StochasticParameter, got %s." % (type(cval),))
        ...
        if mode == ia.ALL:
            ...
            raise Exception("Expected mode to be img_aug.ALL, a string, a list of strings or StochasticParameter, got %s." % (type(mode),))
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        ...
        for i in xrange(nb_images):
            ...
            scale_x, scale_y = scale_samples[0][0], scale_samples[1][0]
            translate_x, translate_y = translate_samples[0][0], translate_samples[1][0]
            ...
            rotate = rotate_samples[0]
            shear = shear_samples[0]
            cval = cval_samples[0]
            mode = mode_samples[0]
