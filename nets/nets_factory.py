...
def get_network_fn(config):
  ...
  """
  if config.network.network_name not in networks_map:
    raise ValueError('Name of network unknown %s' % config.network.network_name)
  func = networks_map[config.network.network_name]
  @functools.wraps(func)
  def network_fn(images, config, **kwargs):
    arg_scope = arg_scopes_map[config.network.network_name](weight_decay=config.train.weight_decay)
    with slim.arg_scope(arg_scope):
      return func(images, 1000, is_training=config.input.is_train, **kwargs)
